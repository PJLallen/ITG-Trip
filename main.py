...
def get_args_parser():
    ...
    parser.add_argument('--lr_mamba', default=1e-3, type=float)
    ...
    parser.add_argument('--mamba_layers', default=6, type=int,
                        help="Number of mamba layers in the temporal block")
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        mAPi.reset_global()
        recognition1.reset_global()
        mAPi1.reset_global()
        
        # test_stats_rec_ivt, test_stats_rec_i, test_stats_rec_ivt1, test_stats_rec_i1 = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val, 
        #                                                 criterion, args.subject_category_id, device, args, detect, recognition, mAPi, detect_i, recognition1, mAPi1)   
        
        # performance_ivt = test_stats_rec_ivt1['mAP']
        ...
        #             **{f'test_stats_rec_ivt1_{k}': v for k, v in test_stats_rec_ivt1.items() if k not in ['AP', 'Rec', 'Pre']},
        #             **{f'test_stats_rec_i1_{k}': v for k, v in test_stats_rec_i1.items() if k not in ['AP', 'Rec', 'Pre']},
        #             'epoch': epoch,
